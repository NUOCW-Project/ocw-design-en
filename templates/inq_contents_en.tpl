<h3>Inquiries about this project may be directed to:</h3>
<div class="inquiry">
<p>OCW Administrative Office</p>
<p>Center for Information Media Studies</p>
<p>Nagoya University</p>
<p>Furo-cho, Chikusa-ku, Nagoya-shi 464-8603, Japan</p>
</div>

<div class="inquiry">
<p id="tel">Telephone: +81-52-789-3904</p>
<p>E-mail:<img id="mail_adress" class="text-image" src="images/en/mail_address.svg" alt=""></p>
<p>We prefer to receive inquiries by e-mail.</p>
<p>Note: The e-mail address is contained in a graphic file to prevent spamming.</p>
</div>
